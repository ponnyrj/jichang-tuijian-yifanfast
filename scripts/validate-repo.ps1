...
$artifactPath = Join-Path $root 'BEGINNER_PATH.md'
...
if ($firstLine -ne '# 机场推荐 2026：新手第一次购买的六步路线') { $problems.Add('README H1 与立项卡定义不一致。') }
if ([regex]::Matches($readme, '机场推荐').Count -lt 4) { $problems.Add('README 没有形成清晰的“机场推荐”主关键词口径。') }
if ($readme -notmatch [regex]::Escape('](BEGINNER_PATH.md)')) { $problems.Add('README 没有链接独有实用文件。') }

$brandNames = @('U1S1', '光速云', 'sogo云')
foreach ($brandName in $brandNames) {
    if ($readme -notmatch [regex]::Escape($brandName)) { $problems.Add("README 缺少推荐品牌：$brandName。") }
}

$restrictedBrands = @('U1S1', 'sogo云')
...
if ($urls.Count -ne 3) { $problems.Add("README 应有 3 个独立外部导流链接，当前为 $($urls.Count) 个。") }
foreach ($url in $urls) {
    $uri = [Uri]$url
    if ($uri.Scheme -ne 'https' -or $uri.Host -ne 'yifanfast.com') {
        $problems.Add("发现非目标域名或非 HTTPS 外链：$url")
        continue
    }
    foreach ($required in @('utm_source=github', 'utm_medium=referral', 'utm_campaign=jichang-tuijian-yifanfast')) {
...
Write-Host "通过：独立主题、实用文件、3 个品牌和 $($urls.Count) 个 UTM 导流链接均符合发布口径。" -ForegroundColor Green
